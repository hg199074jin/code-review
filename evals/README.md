...
## Documentation sweep (mandatory before every merge)

Three consecutive releases shipped stale doc metadata (MS-V21-08 -> audit-2026-09-21 CR-006 ->
...
4. open-item lists ("Open follow-ups") in READMEs and `release-evals/results.tsv` - close what the
   shipped artifact already closed;
5. line-count claims (`wc -l` vs the run.sh guard's split() count differ by the trailing newline).
