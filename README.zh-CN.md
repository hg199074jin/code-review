...
runtime 所有）。跟进项：独立性封顶的 runtime attestation 实现。已闭合：M 用例归因改造
（`maintenance/harness-attribution` 补丁——每个 M 用例的红必须可归因到目标守卫、变异本身须
证明已应用、裁判自身可驳；`mutation-test.sh` 现 37/37）；s11 于 `015e550` 按方案 2 修复（恢复注入初态，fresh 验收已证
